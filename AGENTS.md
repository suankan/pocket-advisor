...
7. **Secrets and personal content stay outside version control.** Never commit tokens, private endpoints, or real personal content; use placeholders and redacted examples. Use the existing separation rather than creating an exception: `workspaces/` is gitignored for private workspace material. (Store credentials are a fixed local convention now, not a secret — see [README §3](README.md#3-configuration) — but nothing else about a workspace stops being private.)
