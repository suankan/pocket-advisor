...
| MLX embedding backend API-fixed + smoke-tested (generic sentences only — model repo, load/embed call sequence, shape/normalization/discrimination/cross-lingual all verified 2026-07-12) but not yet compared to llama_cpp on the real corpus | Eval harness (1a) lands | Run eval.py compare on both backends before treating MLX as safe for real answers |
