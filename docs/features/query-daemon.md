...
2. The socket is mode `0600`. Requests and responses are newline-delimited
   JSON capped at 1 MiB. The server handles one request at a time because MLX
   backends are not assumed thread-safe.
3. Models, active leaf/thread matrices, and their IDs load once into a typed
   `SearchResources`. Every request still calls the same native `run_search`
   used by cold CLI and the accuracy suite. No duplicate ranking path exists.
...
6. `status` reports the selected workspace, process ID, model/index
