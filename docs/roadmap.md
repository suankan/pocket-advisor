...
`docs/workspace-parsing-design.md` and
...
- **daemon** — session-warm serving of the native relational retriever
  ...
  retired columns.
- **accuracy** — golden-set runner over the native retriever; once available,
  A/B the shipped `envelope-v1` payload against a plain-payload index to
