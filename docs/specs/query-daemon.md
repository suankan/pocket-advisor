...
- Not a cloud service — **localhost Unix socket only**, mode `0600`,
  under `output/` (gitignored, machine-local).
...
| socket/pid paths | `output/query_daemon.sock` / `.pid` | Not user-facing names of case data |

### Security / privacy

- Socket only under local `output/`; not exposed on TCP/LAN.
