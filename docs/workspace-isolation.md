...
The authenticated HTTP MCP topology adds a per-workspace Kubernetes workload and TLS gateway without replacing the storage boundaries. MCP-specific isolation details are described in [MCP server design](mcp.md#workspace-isolation). The broader administrative API and control plane remain target state.
