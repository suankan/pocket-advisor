...
func (t *MailboxTool) DescribeAll() []ToolDefinition {
	...
	return []ToolDefinition{
		{Name: t.ListName(), Title: "List " + title + " email messages", Description: "List exact email messages in this fixed workspace. Filter sender by one exact normalized mailbox or one exact normalized domain; filter recipient only by an exact normalized mailbox. Also filter by date, direction, and order. Results contain server-issued message and conversation references; pass a returned pagination cursor unchanged. This is deterministic mailbox evidence, not generated advice. Do not provide a workspace, SQL, or a constructed reference. Each message reports where to find it relative to the workspace local staging directory on the operator machine: source_path is the message own staged file, and when that is absent container_source_path names the file it was extracted from. Report the path given; never infer one from where similar messages live.", InputSchema: mailboxListInputSchema(), OutputSchema: mailboxListOutputSchema(), Annotations: readOnlyAnnotations()},
		{Name: t.ConversationName(), Title: "Fetch " + title + " conversation", Description: "Fetch every email message in one fixed-workspace conversation in stable chronological order. Pass exactly a server-issued message or conversation reference returned by mailbox browsing. Relationship method, warnings, omissions, and evidence references are included. Do not construct a reference or provide a workspace. Each message reports where to find it relative to the workspace local staging directory on the operator machine: source_path is the message own staged file, and when that is absent container_source_path names the file it was extracted from. Report the path given; never infer one from where similar messages live.", InputSchema: mailboxConversationInputSchema(), OutputSchema: mailboxConversationOutputSchema(), Annotations: readOnlyAnnotations()},
...
func mailboxMessageSchema(extra map[string]any, nullableTime, stringArray map[string]any) map[string]any {
	properties := map[string]any{
		"ref": map[string]any{"type": "string", "minLength": 1}, "doc_id": map[string]any{"type": "string", "minLength": 1}, "conversation_ref": map[string]any{"type": "string", "minLength": 1}, "conversation_id": map[string]any{"type": "string", "minLength": 1}, "conversation_method": map[string]any{"type": "string", "minLength": 1}, "subject": map[string]any{"type": "string"}, "sent_at": nullableTime, "sender": map[string]any{"type": "string"}, "recipients": stringArray, "automated_class": map[string]any{"type": "string"}, "list_id": map[string]any{"type": "string"}, "source_path": map[string]any{"type": "string"}, "container_source_path": map[string]any{"type": "string"}, "conversation": map[string]any{"$ref": "#/$defs/conversation_summary"},
	}
