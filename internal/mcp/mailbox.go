...
func mailboxFilterSchema(includeCursor bool) map[string]any {
	props := map[string]any{"sender": map[string]any{"type": "string", "maxLength": maxMailboxAddressRunes}, "recipient": map[string]any{"type": "string", "maxLength": maxMailboxAddressRunes}, "after": nullableDateSchema(), "before": nullableDateSchema(), "order": map[string]any{"enum": []string{"newest_first", "oldest_first"}}, "limit": map[string]any{"type": "integer", "minimum": 1, "maximum": maxMailboxLimit}, "collapse_conversations": map[string]any{"type": "boolean"}, "direction": map[string]any{"enum": []string{"either", "inbound", "outbound"}}}
	if includeCursor {
