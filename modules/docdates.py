"""Best-effort document date extraction for corpora-native PDFs.
...
Pure module: no config import; the header window size is a parameter.
"""
import re
...
_DMY_NAME = re.compile(
    rf"\b(\d{{1,2}})(?:st|nd|rd|th)?\s+({_MONTH_ALT})\.?,?\s+"
    rf"(\d{{4}}|\d{{2}})\b",
    re.IGNORECASE)
_MDY_NAME = re.compile(
    rf"\b({_MONTH_ALT})\.?\s+(\d{{1,2}})(?:st|nd|rd|th)?,?\s+"
    rf"(\d{{4}}|\d{{2}})\b",
    re.IGNORECASE)
...
class DocumentDate:
    date_iso: str
    source: str            # extracted_text | filename | mtime
    detail: str | None
    raw: str | None

    @property
    def is_weak(self) -> bool:
        """True when the date did not come from the document text."""
        return self.source in ("filename", "mtime")
...
def find_dates(text: str) -> list[tuple[int, date, str]]:
    ...
    for m in _DMY_NAME.finditer(text):
        d = _make_date(m.group(3), _MONTH_TO_NUM.get(m.group(2).lower()),
                       m.group(1))
        if d:
            found.append((m.start(), d, m.group(0)))
    for m in _MDY_NAME.finditer(text):
        d = _make_date(m.group(3), _MONTH_TO_NUM.get(m.group(1).lower()),
                       m.group(2))
        if d:
            found.append((m.start(), d, m.group(0)))
...
def _pick_range_aware(dates: list[tuple[int, date, str]],
                      text: str) -> tuple[date, str]:
...
def extract_document_date(text: str, filename: str, mtime_date_iso: str,
                          header_window_chars: int = 6000) -> DocumentDate:
    """Never returns a missing date — mtime is the final fallback and
    is labeled as such."""
    header = (text or "")[:header_window_chars]
    low = header.lower()
    ...
    for kw in _KEYWORDS:
        ...
        # collapse pdftotext -layout column padding (incl. newlines) or
        # a range's second date can sit past any fixed-size raw window
        window = re.sub(r"\s{2,}", " ", window)
        dates = find_dates(window)
        if dates:
            d, raw = _pick_range_aware(dates, window)
            return DocumentDate(d.isoformat(), "extracted_text",
                                f"keyword:{kw}", raw)
    ...
    if dates:
        ...
        return DocumentDate(d.isoformat(), "extracted_text", "dateline", raw)
    ...
    if m:
        d = _make_date(m.group(1), m.group(2), m.group(3))
        if d:
            return DocumentDate(d.isoformat(), "filename",
                                "filename_yyyymmdd", m.group(0))
    dates = find_dates(filename or "")
    if dates:
        d, raw = _pick_range_aware(dates, filename)  # range -> later date
        return DocumentDate(d.isoformat(), "filename", "filename_scan", raw)

    # 4. full-document scan, first match — last text-based resort
    dates = find_dates(text or "")
    if dates:
        return DocumentDate(dates[0][1].isoformat(), "extracted_text",
                            "fulltext_scan", dates[0][2])

    # 5. mtime — weakest source, always labeled
    return DocumentDate(mtime_date_iso, "mtime", None, None)
