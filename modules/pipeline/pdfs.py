...
class PdfTextStage(Stage):
    ...
    def _collect_native(self, stats: StageStats) -> None:
        ...
        for cand in candidates:
            ...
            self.process_native_candidate(cand, stats)
        progress.done()

    def process_native_candidate(
            self, cand: Candidate, stats: StageStats) -> int | None:
        """Register one native candidate on the coordinator thread."""
        try:
            document_id = self._register_native(cand, stats)
            set_candidate_status(
                self.conn, cand.id, CandidateStatus.INGESTED)
            self.conn.commit()
            return document_id
        except Exception as exc:
            self.conn.rollback()
            ...
            set_candidate_status(
                self.conn, cand.id, CandidateStatus.ERROR)
            self.conn.commit()
            stats.inc("errors")
            return None

    def _register_native(self, cand: Candidate, stats: StageStats) -> int:
...
class StreamingPdfProducer:
    """Incrementally fed PDF transform pool for streaming ``ingest all``.

    Workers own only private temporary outputs. The caller offers document
    identities and polls completions on the coordinator thread, which reuses
    ``PdfTextStage``'s verified publication and readiness-dispatch gates.
    """

    def __init__(self, stage: PdfTextStage, stats: StageStats):
        self.stage = stage
        self.stats = stats
        self.recipes = pdf_recipes(langs=stage.config.ocr_langs)
        self.perf = stage.ctx.telemetry.pdfs
        self.budget = max(1, os.process_cpu_count() or 1)
        self.perf.resources.configured_per_child_jobs = OCR_CHILD_JOBS
        self.perf.resources.configured_global_cpu_budget = self.budget
        self._offered: set[int] = set()
        self._futures: dict[Future, tuple[
            _PendingPdf, TransformRequest, PdfTransformCache]] = {}
        self._executor: ThreadPoolExecutor | None = None
        self._worker_ids: queue.Queue[int] = queue.Queue()
        for worker_id in range(self.budget):
            self._worker_ids.put(worker_id)
        self._progress = stage.log.worker_pool(
            "pdf to text", workers=self.budget, total=0)
        stage.config.runtime_dir.mkdir(parents=True, exist_ok=True)
        self._temp = tempfile.TemporaryDirectory(
            prefix="pdf-stream-", dir=stage.config.runtime_dir)
    ...
    def offer_pending_documents(self) -> int:
        offered = 0
        for doc in self.stage._pdf_documents():
            if self.offer(doc):
                offered += 1
        return offered

    def offer(self, doc: _PendingPdf) -> bool:
        if self._closed:
            raise RuntimeError("cannot offer a PDF after producer close")
        if doc.document_id in self._offered:
            return False
        self._offered.add(doc.document_id)
        self.perf.occurrences_considered = len(self._offered)

        cache = PdfTransformCache(
            self.stage.config.document_artifacts(
                doc.sha256).transforms_dir)
        product = cache.load_text(doc.sha256, self.recipes)
        if self.stage._document_current(
                doc, product, self.recipes.combined):
            self.perf.unchanged_documents += 1
            return True

        self.perf.pending_occurrences += 1
        self.perf.pending_admission_bytes += doc.size_bytes
        if doc.extraction_method not in (
                None, "error", self.recipes.combined):
            self.stats.inc("recipe_stale")
        if product is not None:
            self.perf.duplicate_reuses += 1
            self.stage._settle_document(
                doc, product, None, self.recipes.combined, self.stats)
            return True

        source_path = self.stage._verified_document_source(doc)
        if source_path is None:
            self.perf.failed_transforms += 1
            self.stage._settle_document(
                doc, None,
                "IntegrityError: document source copy no longer matches"
                " its recorded SHA-256",
                self.recipes.combined, self.stats)
            return True

        if self._executor is None:
            self._executor = ThreadPoolExecutor(
                max_workers=self.budget,
                thread_name_prefix="pdf-transform")
            self._started_at = time.monotonic()
        index = self.perf.unique_transforms
        request = TransformRequest(
            document_id=doc.document_id,
            source_sha256=doc.sha256,
            source_path=source_path,
            recipes=self.recipes,
            cached_ocr=cache.load_ocr(doc.sha256, self.recipes.ocr),
            work_dir=self._work_root /
            f"job-{index:06d}-{doc.sha256[:12]}",
            langs=self.stage.config.ocr_langs,
            source_bytes=doc.size_bytes,
            queued_at=time.monotonic(),
        )
        self.perf.unique_transforms += 1
        self._progress.add_total()
        future = self._executor.submit(self._worker, request)
        self._futures[future] = (doc, request, cache)
        return True

    def _worker(self, request: TransformRequest) -> TransformResult:
        if is_interrupted():
            raise InterruptedError("PDF transform interrupted")
        worker_id = self._worker_ids.get()
        note = f"transform {request.source_sha256[:12]}"
        self._progress.begin(worker_id, note)
        try:
            ...
            self._progress.finish(worker_id, note)
            self._worker_ids.put(worker_id)

    def poll(self, *, block: bool = False) -> int:
        """Settle currently complete work on the caller/coordinator thread."""
        if not self._futures:
            return 0
        completed = {
            future for future in self._futures if future.done()}
        if not completed and block:
            completed, _ = wait(
                set(self._futures), return_when=FIRST_COMPLETED)
        settled = 0
        for future in sorted(
                completed,
                key=lambda item: self._futures[item][0].document_id):
            doc, request, cache = self._futures.pop(future)
            result = future.result()
            self.perf.timings_seconds.queue_wait_total += \
                result.queue_wait_seconds
            product, error = self.stage._publish_transform_result(
                request, result, self.recipes, cache)
            self.stage._settle_document(
                doc, product, error, self.recipes.combined, self.stats)
            settled += 1
        return settled
    ...
    def close(self) -> None:
        ...
        try:
            ...
            self._progress.done()
            self._temp.cleanup()
            self.perf.resources.configured_worker_count = min(
                self.budget, self.perf.unique_transforms)
            self.perf.resources.observed_peak_workers = self._peak
            if self._started_at is not None:
                self.perf.timings_seconds.transform_wall += \
                    time.monotonic() - self._started_at

    def abort(self) -> None:
        """Cancel queued transforms and clean private runtime state."""
        if self._closed:
            return
        request_interrupt()
        cancel_active_commands()
